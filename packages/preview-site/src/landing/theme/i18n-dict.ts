/**
 * @file i18n-dict.ts
 * @description Pure translation data for the landing page (two locales).
 * Extracted from `i18n.ts` so the hook stays a thin logic module and this
 * large key/value table is a declared data asset (see .vibe-riskrc.json).
 * Every key present in `zh` must also be present in `en` — the parity
 * guard that enforces it lives next to the `I18nKey` type in `i18n.ts`.
 */
...
export const DICT = {
  zh: {
    ...
    'changelog.empty.missingSubtitle':
      '在 src/lib/site-config.ts 里把 owner / repo 改成你实际的公开仓库（私有仓库匿名访问会拿到相同的 404）。',
  ...
  en: {
    ...
    'changelog.empty.missingSubtitle':
      'Update owner / repo in src/lib/site-config.ts to point at your real public repository (private repos return the same 404 anonymously).',
