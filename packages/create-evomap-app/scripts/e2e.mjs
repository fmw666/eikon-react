// End-to-end test for create-evomap-app.
//
...
const SCENARIOS = [
  ...
  {
    id: 'default',
    projectName: 'evomap-e2e-default',
...
async function main() {
  const tmp = await mkdtemp(path.join(tmpdir(), 'evomap-e2e-'));
...
async function runScenario(scenario, tmpRoot, tarballPath) {
  ...
  // and end up with a working `node_modules/.bin/create-evomap-app`. This is
  ...
  const cliBin = path.join(
    ...
    'node_modules',
    '.bin',
    process.platform === 'win32' ? 'create-evomap-app.cmd' : 'create-evomap-app'
  );
